...
import { defaultImage, themeGlobal } from '@/utility/globalState';
import { user } from '@/types';
import { useAtom } from 'jotai';
import { screenSizeGlobal } from './AtomLoader';
...
export default function NavBar({ seenUser }: { seenUser?: user }) {
    ...
    return (
        <div className={styles.navMainDiv} style={{ display: "flex", justifyContent: "space-between", padding: ".3rem 1rem", backgroundColor: "rgb(var(--backgroundColor))", color: "#fff", alignItems: "center", position: "relative", zIndex: "999" }}>
            <Link href={"/"}>
                    ...
                    (
                        <div style={{ position: "relative" }}>
                            <div style={{ display: "grid", gridTemplateColumns: "auto auto auto", gap: ".3rem", alignItems: "center", cursor: "pointer" }} onClick={() => showingMoreSet(prev => !prev)}>
                                ...
                                {!screenSize.phone && seenUser.name && <p>{seenUser.name}</p>}
                            ...
                            {showingMore &&
                                <div onClick={() => showingMoreSet(false)} style={{ position: "fixed", top: 0, left: 0, backgroundColor: "rgba(0,0,0,0.3)", width: "100vw", height: "100vh" }}>
                                    <ul style={{ position: "absolute", backgroundColor: "rgb(var(--backgroundColor))", width: "min(200px, 50vw)", right: "1rem", top: "3.7rem", padding: "1rem", display: "grid", gap: "1rem" }} onClick={() => showingMoreSet(false)}>
                                        ...
                                        <button onClick={() => { themeSet(prev => !prev) }}>Change theme</button>
                                    </ul>
                                </div>
                            }
                        </div>
